...
def get_current_user():
    ...
    apikey = os.getenv("SUPABASE_KEY") or os.getenv("SUPABASE_KEY_DEV") or os.getenv("SUPABASE_KEY_PROD")
    headers = {"Authorization": f"Bearer {token}"}
    if apikey:
        headers["apikey"] = apikey

    try:
        logger.info("Verifying token with Supabase auth endpoint: %s (token preview=%s)", url, (token[:12] + '...') if token else None)
